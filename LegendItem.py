...
class LegendItem(GraphicsWidget, GraphicsWidgetAnchor):
    ...
    def __init__(self, size=None, offset=None, horSpacing=25, verSpacing=0, pen=None,
                 brush=None, labelTextColor=None, textSize=None,
                 textBold=None, textItalic=None, **kwargs):
        """
        ==============  ===============================================================
        **Arguments:**
        ...
        self.opts = {
            'pen': fn.mkPen(pen) if pen else fn.mkPen(255, 255, 255, 100),
            'brush': fn.mkBrush(brush) if brush else fn.mkBrush(100, 100, 100, 50),
            'labelTextColor': labelTextColor,
            # FIXME: Should these be lableTextSize, etc?
            'textSize': textSize,
            'textBold': textBold,
    ...
    def labelItemOptions(self):
        ...
        def _translateArg(arg):
            for p in ["labelText", "text"]:
                if arg.starswith(p):
                    arg = arg.replace(p, "")
            return arg

        arglist = {_translateArg(k): v for k, v in self.opts.items()
                   if _translateArg(k) != k and v is not None}
        return arglist
    ...
    def setTextSize(self, size):
        self.opts["textSize"] = labelOptions["size"] = size

    def setTextBold(self, bold):
        self.opts["textBold"] = lableOptions["bold"] = bold

    def setTextItalic(self, italic):
        self.opts["textItalic"] = lableOptions["italic"] = italic

    def setPen(self, pen):
        self.opts["pen"] = fn.mkPen(pen)

    def setBrush(self, brush):
        self.opts["pen"] = fn.mkBrush(brush)

    def setOpacity(self, level):
        self.opts["brush"].setAlpha(level)
